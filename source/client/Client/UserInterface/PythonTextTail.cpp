...
void CPythonTextTail::ShowAllTextTail()
{
	...
	for (itor = m_CharacterTextTailMap.begin(); itor != m_CharacterTextTailMap.end(); ++itor)
	{
		...
		if (pTextTail->fDistanceFromPlayer < 2000.0f)
	...
	{
		TTextTail* pTextTail = itor->second;
		pTextTail->bRender = pTextTail->fDistanceFromPlayer < 2000.f;
	}
#endif
}
...
#endif
{
	...
	if (0 != dwGuildID)
	{
		...
		if (!CPythonGuild::Instance().GetGuildName(dwGuildID, &strGuildName))
			strGuildName = "Noname";
		if (dwNewIsGuildName == 4)
			strGuildName.insert(0, "|cfff7b838 [Ronark Hakimi] |cffffffff- [Lonca Lideri] - ");
		else if (dwNewIsGuildName == 3)
			strGuildName.insert(0, "[Lonca Lideri] - ");
		else if (dwNewIsGuildName == 2)
			strGuildName.insert(0, "|cfff7b838 [Ronark Hakimi]|cffffffff - ");
#else
		std::string strGuildName;
		if (!CPythonGuild::Instance().GetGuildName(dwGuildID, &strGuildName))
			strGuildName = "Noname";
