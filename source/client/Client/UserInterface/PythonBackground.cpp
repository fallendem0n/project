...
void CPythonBackground::CreateTargetEffectShop(DWORD dwID, DWORD dwChrVID)
{
	m_kMap_dwTargetID_dwChrShopID.insert(make_pair(dwID, dwChrVID));
}
void CPythonBackground::CreateTargetEffectShop(DWORD dwID, long lx, long ly)
{
	if (m_kMap_dwTargetID_dwChrShopID.end() != m_kMap_dwTargetID_dwChrShopID.find(dwID))
		return;
...
#if defined(ENABLE_SHOP_SEARCH) && defined(ENABLE_OFFLINE_SHOP)
	//search shop effect
	std::map<DWORD, DWORD>::iterator itor2 = m_kMap_dwTargetID_dwChrShopID.begin();
	for (; itor2 != m_kMap_dwTargetID_dwChrShopID.end(); ++itor2)
	{
...
#if defined(ENABLE_SHOP_SEARCH) && defined(ENABLE_OFFLINE_SHOP)
	m_kMap_dwTargetID_dwChrShopID.clear();
#endif
...
void CPythonBackground::DeleteTargetEffect(DWORD dwID)
{
	...
	if (m_kMap_dwTargetID_dwChrShopID.end() != m_kMap_dwTargetID_dwChrShopID.find(dwID))
	{
		m_kMap_dwTargetID_dwChrShopID.erase(dwID);
	}
#endif
