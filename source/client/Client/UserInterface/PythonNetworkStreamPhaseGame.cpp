...
void CPythonNetworkStream::GamePhase()
{
    ...
    while (ret)
	{
		...
		switch (header)
		{
			...
			case HEADER_GC_NEW_OFFLINESHOP://offlineshop
...
#ifdef SHOP_STOK
		case SHOP_SUBHEADER_GC_STOK:
			PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "OnShopError", Py_BuildValue("(s)", "NOT_ENOUGH_STOK"));
			break;
#endif

#ifdef PRICE_TYPE
		case SHOP_SUBHEADER_GC_NOT_ENOUGH_MONEY_EX_EP:
			PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "OnShopError", Py_BuildValue("(s)", "NOT_ENOUGH_MONEY_EX_EP"));
			break;
...
bool CPythonNetworkStream::RecvTargetCreatePacketNew()
{
	...
	else if (CREATE_TARGET_TYPE_SHOP == kTargetCreate.byType)
	{
		...
		rkpyBG.CreateTargetEffectShop(kTargetCreate.lID, kTargetCreate.dwVID);
	}
#endif
	else
	{
...
bool CPythonNetworkStream::SendSwitchbotStartPacket(BYTE slot, BYTE bBotSpeed,   std::vector<CPythonSwitchbot::TSwitchbotAttributeAlternativeTable> alternatives)
{
	...
	if (!Send(sizeof(bBotSpeed), &bBotSpeed))
	{
		return false;
	}
