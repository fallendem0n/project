...
int CPythonSwitchbot::GetAttibuteSet(BYTE slot)
{
	...
	if (item_type == CItemData::ITEM_TYPE_WEAPON)
	{
		...
		else if (item_subtype == CItemData::WEAPON_QUIVER)
			return -1;
#endif
...
PyObject* switchbotStart(PyObject* poSelf, PyObject* poArgs)
{
	...
	BYTE bBotSpeed;
	if (!PyTuple_GetByte(poArgs, 1, &bBotSpeed))
	{
		return Py_BuildException();
	}
	
	if (bBotSpeed < 0 || bBotSpeed > 5)
		bBotSpeed = 0;
	...
	CPythonNetworkStream::Instance().SendSwitchbotStartPacket(bSlot, bBotSpeed, vec_alternatives);
	return Py_BuildNone();
}
...
void initSwitchbot()
{
	...
	PyModule_AddIntConstant(poModule, "SWITCH_SPEED_COUNT", SWITCH_SPEED_COUNT);
}
