...
void CPythonNetworkStream::ServerCommand(char * c_szCommand)
{
	...
	else if (!strcmpi(szCmd, "BINARY_SwitchBotSpeedReload"))
	{
		...
		DWORD botSpeed = (DWORD)atoi(TokenVector[1].c_str());
		PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_SwitchBotSpeedReload", Py_BuildValue("(i)", botSpeed));
	}
#endif
	// GIFT NOTIFY
