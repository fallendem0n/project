...
typedef struct SShopItemTable
{
	...
	TItemPos	pos;			// PC »ó?z^¸¸ REz?	#ifdef FULL_YANG
	#ifdef FULL_YANG
	long long		price;	// PC, shop_table_ex.txt »ó?z^¸¸ REz?	#else
	#else
	DWORD		price;	// PC, shop_table_ex.txt »ó?z^¸¸ REz?	#endif
	#endif
#ifdef ENABLE_CHEQUE_SYSTEM
	DWORD		cheque;
#endif
	...
	DWORD		stok;
#endif
	BYTE		display_pos; // PC, shop_table_ex.txt »ó?z^¸¸ REz? þ?D R§Ä^.
} TShopItemTable;
...
#ifdef ENABLE_CHEQUE_SYSTEM
			total += (long long) YANG_PER_CHEQUE * (int) iCheque;
#endif
			return total;
		}
	} TPriceInfo;
