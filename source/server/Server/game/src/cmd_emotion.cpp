...
ACMD(do_emotion)
{
//Atak Yapaken Duygularý Kullanma Fix
bool bAttacking = (get_dword_time() - ch->GetLastAttackTime()) < 750;
bool bMoving = (get_dword_time() - ch->GetLastMoveTime()) < 750;
//Atak Yapaken Duygularý Kullanma Fix
	int i;
	{
		if (ch->IsRiding())
...
//Atak Yapaken Duygularý Kullanma Fix
		if (bAttacking || bMoving)
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Duygularý kullanmak için hareketsiz durmalýsýn."));
			return;
		}
//Atak Yapaken Duygularý Kullanma Fix
	}
