...
#endif
	DWORD	vnum;
	long	alSockets[ITEM_SOCKET_MAX_NUM];	// LN횯arCL
...
#ifdef BLACKMARKET
		bool            BlackMarket(TItemPos Cell);
#endif
...
#ifdef LUCK_BOX
		void				SetBoxVID(DWORD vid) { m_luckboxvid = vid; };
		DWORD				GetBoxVID() { return m_luckboxvid; };
#endif
...
public:
	...
	private:
		std::vector<int>	R_I_SlotIndex;
	public:
		void				R_I_AddItemSlot(int ItemSlot) { R_I_SlotIndex.push_back(ItemSlot); }
		std::vector<int> & R_I_GetSlotIndex() { return R_I_SlotIndex; }
#endif
