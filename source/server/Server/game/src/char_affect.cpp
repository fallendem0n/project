...
{
//Büyülü Silah Ve Hava Kýlýcý Bug Fix
...
bool CHARACTER::AddAffect(DWORD dwType, BYTE bApplyOn, long lApplyValue, DWORD dwFlag, long lDuration, long lSPCost, bool bOverride, bool IsCube )
{
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("¿î¿µÀÚ Á¦Á¦·Î Ã¤ÆÃÀÌ ±ÝÁö µÇ¾ú½À´Ï´Ù."));
	}
	// END_OF_CHAT_BLOCK
...
bool CHARACTER::RemoveAffect(CAffect* pkAff)
{
	...
	if (AFFECT_REVIVE_INVISIBLE != pkAff->dwType && AFFECT_MOUNT != pkAff->dwType)//BÝNEKLERÝN GERÝYE ATMASI FÝX
		ComputePoints();
	else
		UpdatePacket();
...
bool CHARACTER::RemoveAffect(DWORD dwType)
{
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Ã¤ÆÃ ±ÝÁö°¡ Ç®·È½À´Ï´Ù."));
	}
	// END_OF_CHAT_BLOCK
