...
#ifdef BLACKMARKET
void CInputMain::BlackMarket(LPCHARACTER ch, const char * data)
{
    struct command_item_blackmarket * pinfo = (struct command_item_blackmarket *) data;
    if (ch)
        ch->BlackMarket(pinfo->Cell);
}
#endif
...
/*#ifdef URIEL_ANTI_CHEAT
	...
	unsigned long a = 534423269, b = 556690549, c = pinfo->uX, d = pinfo->uY;
...
/*#ifdef URIEL_ANTI_CHEAT
	// Hicbir zaman herhangi biri ile asagidaki sayilari paylasmayiniz.
	// Sayilari el ile degistirmeye calismayiniz, tum yapiyi bozabilirsiniz.
	unsigned long a = 1648800397, b = 2425522283;
...
//Kemer Bug Fix
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "?ce kemer envanterini boþalt!");
		return;
	}
//Kemer Bug Fix
...
//Giyili ?emi Dopoya Koyma Fix
	if (true == pkItem->IsEquipped())
	{
	ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("giyili_item_fixed"));
	return;
	}
//Giyili ?emi Dopoya Koyma Fix
	...
	&& !pkItem->IsUpgradeItem() && !pkItem->IsBook() && !pkItem->IsStone()
#endif
	)
...
int OfflineshopPacketCreateNewShop(LPCHARACTER ch, const char* data, int iBufferLeft)
{
	...
	//fix flooding
	if (rShopInfo.dwCount > 36 || rShopInfo.dwCount == 0) {
		sys_err("tried to open a shop with 36+ items.");
		return -1;
	}
...
void CInputMain::BulkWhisperManager(LPCHARACTER ch, const char* c_pData)
{
	...
	DBManager::instance().Query(
		"INSERT DELAYED INTO `log`.`pm_all_send_log%s` (`sender_pid`, `time`, `ip`, `content`, `channel`) "
		"VALUES (%u, NOW(), '%s', '%s', %d);",
...
#ifdef BLACKMARKET	
		case HEADER_CG_BLACKMARKET:
            if (!ch->IsObserverMode())
                BlackMarket(ch, c_pData);
			break;
#endif
...
int CInputMain::Switchbot(LPCHARACTER ch, const char* data, size_t uiBytes)
{
	...
	case SUBHEADER_CG_SWITCHBOT_START:
	{
		...
		{
			const TSwitchbotAttributeAlternativeTable* pAttr = reinterpret_cast<const TSwitchbotAttributeAlternativeTable*>(c_pData+1);
			c_pData += sizeof(TSwitchbotAttributeAlternativeTable);

			vec_alternatives.emplace_back(*pAttr);
		}

		CSwitchbotManager::Instance().Start(ch->GetPlayerID(), p->slot, bBotSpeed, vec_alternatives);
		return extraLen;
	}
