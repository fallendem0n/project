...
void CHARACTER::Destroy()
{
	...
	if (GetMapIndex() == CRonarkSystem::instance().RONARK_MAP_VNUM)
		CRonarkSystem::instance().LoncaOnlineSil(this);
#endif
#ifdef __ENABLE_NEW_OFFLINESHOP__
...
#ifdef ENABLE_GUILD_LEADER
			if (GetGuild() != NULL)
			{	
				addPacket.dwGuildID = GetGuild()->GetID();
				CGuild* pGuild = this->GetGuild();
				
				if (pGuild->GetMasterPID() == GetPlayerID() && pGuild->GetID() == (DWORD)quest::CQuestManager::instance().GetEventFlag("ronark_king"))
					addPacket.dwNewIsGuildName = 4;

				else if (pGuild->GetMasterPID() == GetPlayerID())
					addPacket.dwNewIsGuildName = 3;

				//else if (pGuild->NewIsGuildGeneral(GetPlayerID()) == true)
				else if (pGuild->GetID() == (DWORD)quest::CQuestManager::instance().GetEventFlag("ronark_king"))
...
#ifdef ENABLE_GUILD_LEADER
	auto pGuild = this->GetGuild();
	if (pGuild)
	{
		if (pGuild->GetMasterPID() == GetPlayerID() && pGuild->GetID() == (DWORD)quest::CQuestManager::instance().GetEventFlag("ronark_king"))
			pack.dwNewIsGuildName = 4;
		else if (pGuild->GetMasterPID() == GetPlayerID())
...
//Cinsiyet De?þtirince Oyundan Atma Fix
	UpdatePacket();
	SET_BIT(m_bAddChrState, ADD_CHARACTER_STATE_SPAWN);
	m_afAffectFlag.Set(AFF_SPAWN);
	ViewReencode();
...
#ifdef ENABLE_BOSS_HASAR_SIRALAMA
	if (CBossHasarSiralama::instance().BossVnum(this->GetRaceNum()))
	{
		return;
	}
#endif
...
//Level Atlay?ca Verilen Potlar Fix
//				if (GetLevel() <= 10)
...
#endif
#ifdef NEW_PVM_APPLYS
		case POINT_METIN_BONUS :
		case POINT_BOSS_BONUS :
		case POINT_DUNGEON_BONUS :
#endif
...
#endif
#ifdef NEW_PVM_APPLYS
		case APPLY_METIN_BONUS:			// 86
		case APPLY_BOSS_BONUS:			// 86
		case APPLY_DUNGEON_BONUS:			// 86
#endif
			PointChange(aApplyInfo[bApplyType].bPointType, iVal);
			break;
...
//Moblar G?enli Alana Giremez Fix
void CHARACTER::OnMove(bool bIsAttack)
{
	...
	//Canavarlar guvenli alana giremez.
...
#endif
		{
			// 2005.03.17.myevan.L¸°URE lC? ?ze´Â ?R?»ó? Al¸® ±â´ÉR?RU??L¸´U.
			if (GetMyShop())
			{
				if (pkChrCauser->IsDead() == true) return;

				//PREVENT_TRADE_WINDOW
				if (pkChrCauser == this) // RÚ±â´??´É
				{
				...
				{
					// L¬¸Z? »ç¶÷RE ?CZ/A^°í/?RÎ»óÁ^/»ó?REzëÁ?E¶ó¸é þN?
					...
					// L¬¸Z? ´ë»óRE ?CZ/A^°í/»ó?REzëÁ?E¶ó¸é þN?
...
//Gardiyan Mob G??ce Hareketlenmesi Fix
//	if (IsGuardNPC())
//	{
//		if (5000 < DISTANCE_APPROX(m_pkMobInst->m_posLastAttacked.x - GetX(), m_pkMobInst->m_posLastAttacked.y - GetY()))
//			if (Return())
...
void CHARACTER::SortInven(BYTE option)
{
	...
	if (GetLastSortTime() > get_global_time()) {
		ChatPacket(CHAT_TYPE_INFO, "Hizli islem yapiyorsun. %d saniye bekle.", GetLastSortTime() - get_global_time());
		return;
	}
