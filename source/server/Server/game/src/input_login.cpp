...
void CInputLogin::Entergame(LPDESC d, const char * data)
{
	...
	if (ch->GetMapIndex() == CRonarkSystem::instance().RONARK_MAP_VNUM){
		CRonarkSystem::instance().GirenLonca(ch);
	}
#endif
...
#ifdef BLACKMARKET
	if (quest::CQuestManager::instance().GetEventFlag("blackmarket") == 1)
	{
		ch->ChatPacket(CHAT_TYPE_COMMAND, "blackmarket_icon");
	}
#endif
#ifdef NEW_SALES_SYSTEM
	if (CSales::instance().CheckSales() && quest::CQuestManager::instance().GetEventFlag("sales") == 1)
		ch->ChatPacket(CHAT_TYPE_COMMAND, "SalesShowIcon");
#endif
//At Level Bug Fix
...
//Analyze: login phase does not handle this packet! header 100 Fix
