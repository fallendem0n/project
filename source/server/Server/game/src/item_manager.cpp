...
LPITEM ITEM_MANAGER::CreateItem(DWORD vnum, DWORD count, DWORD id, bool bTryMagic, int iRarePct, bool bSkipSave)
{
	...
	if (id == 0) 
	{
...
#ifdef LUCK_BOX
		if (vnum == LUCKBOX_VNUM)
		{
			CLuckBox::instance().SetReward(item);
		}
#endif
#ifdef ICE_FIRE_RING
		if (vnum == FIRE_RING)
		{
			...
			{
				item->SetForceAttribute(i,FireRingAttrTable[i][0],FireRingAttrTable[i][1]); /// Ring Level
			}
			
			item->SetSocket(3,1); /// Ring Level
			item->SetSocket(4,0); /// Current Exp
			item->SetSocket(5,FireRingExpTable[1]);  /// Needed Exp
		}
		else if (vnum == ICE_RING)
		{
			
			for (int i = 0; i < _countof(IceRingAttrTable); i++)
			{
				item->SetForceAttribute(i,IceRingAttrTable[i][0],IceRingAttrTable[i][1]); /// Ring Level
			}
			
			item->SetSocket(3,1); /// Ring Level
			item->SetSocket(4,0); /// Current Exp
			item->SetSocket(5,IceRingExpTable[1]);  /// Needed Exp
		}
#endif
...
void ITEM_MANAGER::CreateQuestDropItem(LPCHARACTER pkChr, LPCHARACTER pkKiller, std::vector<LPITEM> & vec_item, int iDeltaPercent, int iRandRange)
{
	...
	if (quest::CQuestManager::instance().GetEventFlag("drop_moon"))
	{
		...
		if (iDropPerKill[pkChr->GetMobRank()])
		{
			...
			if (iPercent >= number(0, iRandRange))
			{
	...
	if (GetDropPerKillPct(100, 2000, iDeltaPercent, "2006_drop") >= number(0, iRandRange))
	{
	...
	if (GetDropPerKillPct(100, 2000, iDeltaPercent, "2007_drop") >= number(0, iRandRange))
	{
		sys_log(0, "육각보합 DROP EVENT ");
	...
	if (GetDropPerKillPct(/* minimum */ 100, /* default */ 1000, iDeltaPercent, "newyear_fire") >= number(0, iRandRange))
	{
	...
	if (GetDropPerKillPct(100, 500, iDeltaPercent, "newyear_moon") >= number(0, iRandRange))
	{
	...
	if (GetDropPerKillPct(1, 2000, iDeltaPercent, "valentine_drop") >= number(0, iRandRange))
	{
	...
	if (GetDropPerKillPct(100, 2000, iDeltaPercent, "icecream_drop") >= number(0, iRandRange))
	{
	...
	if ((pkKiller->CountSpecifyItem(53002) > 0) && (GetDropPerKillPct(50, 100, iDeltaPercent, "new_xmas_event") >= number(0, iRandRange)))
	{
		const static DWORD xmas_sock = 50010;
		pkKiller->AutoGiveItem (xmas_sock, 1);
	}

	if ((pkKiller->CountSpecifyItem(53007) > 0) && (GetDropPerKillPct(50, 100, iDeltaPercent, "new_xmas_event") >= number(0, iRandRange)))
	{
		const static DWORD xmas_sock = 50010;
		pkKiller->AutoGiveItem (xmas_sock, 1);
	}
	...
	if ( GetDropPerKillPct(100, 2000, iDeltaPercent, "halloween_drop") >= number(0, iRandRange) )
	{
	...
	if ( GetDropPerKillPct(100, 2000, iDeltaPercent, "ramadan_drop") >= number(0, iRandRange) )
	{
	...
	if ( GetDropPerKillPct(100, 2000, iDeltaPercent, "easter_drop") >= number(0, iRandRange) )
	{
	...
	if ( GetDropPerKillPct(100, 2000, iDeltaPercent, "football_drop") >= number(0, iRandRange) )
	{
	...
	if (GetDropPerKillPct(100, 2000, iDeltaPercent, "whiteday_drop") >= number(0, iRandRange))
	{
	...
	{
		if (GetDropPerKillPct(100, 1000, iDeltaPercent, "kids_day_drop_high") >= number(0, iRandRange))
		{
	...
	{
		if (GetDropPerKillPct(100, 1000, iDeltaPercent, "kids_day_drop") >= number(0, iRandRange))
		{
			DWORD ITEM_QUIZ_BOX = 50034;
	...
	if (pkChr->GetLevel() >= 30 && GetDropPerKillPct(50, 100, iDeltaPercent, "medal_part_drop") >= number(0, iRandRange))
	{
	...
	if (pkChr->GetLevel() >= 40 && pkChr->GetMobRank() >= MOB_RANK_BOSS && GetDropPerKillPct(/* minimum */ 1, /* default */ 1000, iDeltaPercent, "three_skill_item") / GetThreeSkillLevelAdjust(pkChr->GetLevel()) >= number(0, iRandRange))
	{
	...
	//
	if (GetDropPerKillPct(100, 1000, iDeltaPercent, "dragon_boat_festival_drop") >= number(0, iRandRange))
	{
