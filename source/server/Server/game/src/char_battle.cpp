...
void CHARACTER::DeathPenalty(BYTE bTown)
{
	...
	if (GetLevel() < 10)
	{
		...
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("øÎΩ≈¿« ∞°»£∑Œ ∞Ê«Ëƒ°∞° ∂≥æÓ¡ˆ¡ˆ æ æ“Ω¿¥œ¥Ÿ."));
		return;
	}
...
#ifdef ENABLE_RONARK_SYSTEM
			if (GetMapIndex() == CRonarkSystem::instance().RONARK_MAP_VNUM && GetRaceNum() == CRonarkSystem::instance().RONARK_BOSS_VNUM)	
				item->SetRonarkItem();
#endif
...
void CHARACTER::Dead(LPCHARACTER pkKiller, bool bImmediateDead)
{
	...
	m_dwKillerPID = 0; // π›µÂΩ√ √ ±‚»≠ «ÿæﬂ«‘ DO NOT DELETE THIS LINE UNLESS YOU ARE 1000000% SURE
	...
	if (pkKiller && pkKiller->IsPC())
	{
		if (IsPC() == false)
		{
			if (abs((GetLevel())-(pkKiller->GetLevel())) <= EXP_LEVEL_DIFF)
			{
				if (pkKiller->GetDungeon() == false)
				{
					if (IsStone())
					{
						
						LPITEM ring1 = pkKiller->GetWear(WEAR_SPECIAL_RING1);
						LPITEM ring2 = pkKiller->GetWear(WEAR_SPECIAL_RING2);

						if (ring1 && (ring1->GetVnum() == ICE_RING))
						{
							int currentlevel = ring1->GetSocket(3);
							int currentexp = ring1->GetSocket(4);
							if (currentlevel < 20)
							{
								if (currentexp+1 >= IceRingExpTable[currentlevel])
								{
									
									for (int i = 0; i < _countof(IceRingAttrTable); i++)
									{
										ring1->SetForceAttribute(i,IceRingAttrTable[i][0],IceRingAttrTable[i][1]+((currentlevel+1)*IceRingAttrTable[i][2])); /// Ring Attr
									}
									
									ring1->SetSocket(3,currentlevel+1);
									ring1->SetSocket(4,0);
									ring1->SetSocket(5,IceRingExpTable[currentlevel+1]);
									pkKiller->ComputePoints();
									pkKiller->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ice_ring_level_up_%d"),currentlevel+1);
								}
								else
								{
									ring1->SetSocket(4, currentexp+1);
								}
							}
						}
						else if (ring2 && (ring2->GetVnum() == ICE_RING))
						{
							int currentlevel = ring2->GetSocket(3);
							int currentexp = ring2->GetSocket(4);
							if (currentlevel < 20)
							{
								if (currentexp+1 >= IceRingExpTable[currentlevel])
								{
									for (int i = 0; i < _countof(IceRingAttrTable); i++)
									{
										ring2->SetForceAttribute(i,IceRingAttrTable[i][0],IceRingAttrTable[i][1]+((currentlevel+1)*IceRingAttrTable[i][2])); /// Ring Attr
									}
									
									
									ring2->SetSocket(3,currentlevel+1);
									ring2->SetSocket(4,0);
									ring2->SetSocket(5,IceRingExpTable[currentlevel+1]);
									pkKiller->ComputePoints();
									pkKiller->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ice_ring_level_up_%d"),currentlevel+1);
								}
								else
								{
									ring2->SetSocket(4, currentexp+1);
								}
							}
						}
					}
				...
				{
					if (GetMobRank() >= MOB_RANK_BOSS && IsStone() == false)
					{
						
						LPITEM ring1 = pkKiller->GetWear(WEAR_SPECIAL_RING1);
						LPITEM ring2 = pkKiller->GetWear(WEAR_SPECIAL_RING2);
						
						if (ring1 && (ring1->GetVnum() == FIRE_RING))
						{
							int currentlevel = ring1->GetSocket(3);
							int currentexp = ring1->GetSocket(4);
							if (currentlevel < 20)
							{
								if (currentexp+1 >= FireRingExpTable[currentlevel])
								{
									for (int i = 0; i < _countof(FireRingAttrTable); i++)
									{
										ring1->SetForceAttribute(i,FireRingAttrTable[i][0],FireRingAttrTable[i][1]+((currentlevel+1)*FireRingAttrTable[i][2])); /// Ring Attr
									}
									
									
									ring1->SetSocket(3,currentlevel+1);
									ring1->SetSocket(4,0);
									ring1->SetSocket(5,FireRingExpTable[currentlevel+1]);
									pkKiller->ComputePoints();
									pkKiller->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("fire_ring_level_up_%d"),currentlevel+1);
								}
								else
								{
									ring1->SetSocket(4, currentexp+1);
						...
						else if (ring2 && (ring2->GetVnum() == FIRE_RING))
						{
							int currentlevel = ring2->GetSocket(3);
							int currentexp = ring2->GetSocket(4);
							if (currentlevel < 20)
							{
								if (currentexp+1 >= FireRingExpTable[currentlevel])
								{
									...
									for (int i = 0; i < _countof(FireRingAttrTable); i++)
									{
										ring2->SetForceAttribute(i,FireRingAttrTable[i][0],FireRingAttrTable[i][1]+((currentlevel+1)*FireRingAttrTable[i][2])); /// Ring Attr
									}
									
									
									ring2->SetSocket(3,currentlevel+1);
									ring2->SetSocket(4,0);
									ring2->SetSocket(5,FireRingExpTable[currentlevel+1]);
									pkKiller->ComputePoints();
									pkKiller->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("fire_ring_level_up_%d"),currentlevel+1);
								}
								else
...
#ifdef ENABLE_RONARK_SYSTEM
	if (pkKiller && pkKiller->GetGuild() && this->GetRaceNum() == CRonarkSystem::instance().RONARK_BOSS_VNUM && pkKiller->GetMapIndex() == CRonarkSystem::instance().RONARK_MAP_VNUM)
	{
		CRonarkSystem::instance().VerileriGonder(true); //olduren id
		CRonarkSystem::instance().RonarkOlduAmk(this, pkKiller);
	}
#endif
			...
			else
			{
				if (!isAgreedPVP && !isUnderGuildWar && !IsKillerMode() && GetAlignment() >= 0 && !isDuel && !isForked)
				{
					...
					if (number(1, 100) < iNoPenaltyProb)
						pkKiller->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("øÎΩ≈¿« ∫∏»£∑Œ æ∆¿Ã≈€¿Ã ∂≥æÓ¡ˆ¡ˆ æ æ“Ω¿¥œ¥Ÿ."));
					else
					{
		...
		if (!IS_SET(m_pointsInstant.instant_flag, INSTANT_FLAG_NO_REWARD))
		{
			...
			else
			{
				if (pkKiller->m_dwUnderGuildWarInfoMessageTime < get_dword_time())
				{
					...
					pkKiller->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<±ÊµÂ> ±ÊµÂ¿¸¡ﬂø°¥¬ ªÁ≥…ø° µ˚∏• ¿Ã¿Õ¿Ã æ¯Ω¿¥œ¥Ÿ."));
				}
			}
		}
	}
...
bool CHARACTER::Damage(LPCHARACTER pAttacker, int dam, EDamageType type) // returns true if dead
{
	...
	if (pAttacker)
	{
		if (pAttacker->IsPC())
		{
			...
			{
				if (pAttacker->IsQuestWindow == true)
					return false;
			}
		}
	}
		...
		if (pAttacker)
		{
			...
			if (iPenetratePct)
			{
				...
				if (number(1, 100) <= iPenetratePct)
				{
					...
					if (test_server)
						ChatPacket(CHAT_TYPE_INFO, LC_TEXT("∞¸≈Î √ﬂ∞° µ•πÃ¡ˆ %d"), GetPoint(POINT_DEF_GRADE) * (100 + GetPoint(POINT_DEF_BONUS)) / 100);
	...
	else if (type == DAMAGE_TYPE_NORMAL || type == DAMAGE_TYPE_NORMAL_RANGE)
	{
		...
		else if (type == DAMAGE_TYPE_NORMAL_RANGE)
		{
			...
			if (GetPoint(POINT_DODGE) && number(1, 100) <= GetPoint(POINT_DODGE))
			{
				if (test_server)
				{
					pAttacker->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%s »∏««! (%d%%)"), GetName(), GetPoint(POINT_DODGE));
					ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%s »∏««! (%d%%)"), GetName(), GetPoint(POINT_DODGE));
				}
...
#ifdef ENABLE_RONARK_SYSTEM
	if (GetRaceNum() == CRonarkSystem::instance().RONARK_BOSS_VNUM && pAttacker->GetMapIndex() == CRonarkSystem::instance().RONARK_MAP_VNUM && pAttacker->GetGuild() != NULL)
	{
		float BPercent = (double)(dam) / (double)(GetMobTable().dwMaxHP) * 100;
		CRonarkSystem::instance().LoncaHasarEkle(pAttacker->GetGuild()->GetID(), BPercent);
	}
#endif
		...
		if (m_bDetailLog)
		{
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%s[%d]∞° ∞¯∞› ¿ßƒ°: %d %d"), pAttacker->GetName(), (DWORD) pAttacker->GetVID(), pAttacker->GetX(), pAttacker->GetY());
		}
	}
...
static void GiveExp(LPCHARACTER from, LPCHARACTER to, int iExp)
{
	...
	
	{
		...
		if (to->GetMapIndex() >= 660000 && to->GetMapIndex() < 670000)
			iExp += iExp * 20 / 100; // 1.2πË (20%)

		
		if (to->GetPoint(POINT_EXP_DOUBLE_BONUS))
			if (number(1, 100) <= to->GetPoint(POINT_EXP_DOUBLE_BONUS))
				iExp += iExp * 30 / 100; // 1.3πË (30%)
	...
	public:
		...
		void operator () (DWORD dwTargetVID)
		{
			...
			switch (m_bType)
			{
				...
				case SKILL_BIPABU:
				case SKILL_NOEJEON:
				case SKILL_PAERYONG: //?aman ejderha uzaktan vurma fix
				case SKILL_GEOMPUNG:
				case SKILL_SANGONG:
				case SKILL_MAHWAN:
				case SKILL_PABEOB:
				case SKILL_YONGBI: //?man Ejderha At˝˛?›˛lememe Fix
					//case SKILL_CURSE:
					{
						m_me->OnMove(true);
						pkVictim->OnMove();
...
void CHARACTER::SetVictim(LPCHARACTER pkVictim)
{
	...
	else
	{
		if (m_kVIDVictim != pkVictim->GetVID())
			MonsterLog("∞¯∞› ¥ÎªÛ¿ª º≥¡§: %s", pkVictim->GetName());
