...
LPITEM CItem::RemoveFromCharacter()
{
	...
	else
	{
		if (GetWindow() != SAFEBOX && GetWindow() != MALL)
		{
			...
			{
				if (m_wCell >= SPECIAL_INVENTORY_MAX_NUM) //if (m_wCell >= SPECIAL_INVENTORY_MAX_NUM)
...
#ifdef ICE_FIRE_RING
	if ((0 == GetWearFlag() || ITEM_TOTEM == GetType()) && ITEM_COSTUME != GetType() && ITEM_DS != GetType() && ITEM_SPECIAL_DS != GetType() && ITEM_RING != GetType() && ITEM_BELT != GetType() && ITEM_SPECIAL_RING != GetType())
#else
...
#ifdef ICE_FIRE_RING
	else if (GetType() == ITEM_SPECIAL_RING)
	{
		if (GetSubType() == RING_FIRE) 
		{
			
			return WEAR_SPECIAL_RING1;
		}
		else if (GetSubType() == RING_ICE) 
		{
...
#ifdef ENABLE_RONARK_SYSTEM
	if (CRonarkSystem::instance().GetDeadRonarkStatus() == true && ch && ch->IsPC() && ch->GetGuild() && ch->GetMapIndex() == CRonarkSystem::instance().RONARK_MAP_VNUM)
...
#ifdef ENABLE_RONARK_SYSTEM
			event_cancel(&m_pkOwnershipEvent);
			if (m_bRonarkItem == true && CRonarkSystem::instance().GetRonarkStatus() == 1 && CRonarkSystem::instance().GetRonarkHakimi() != 0)
			{
				CGuild* RonarkGuild = CGuildManager::instance().FindGuild(CRonarkSystem::instance().GetRonarkHakimi());
				if (!RonarkGuild)
				{
					m_dwOwnershipPID = 0;
					TPacketGCItemOwnership p;
				...
				m_dwOwnershipPID = RonarkGuild->GetID();
				item_event_info* info = AllocEventInfo<item_event_info>();
				strlcpy(info->szOwnerName, RonarkGuild->GetName(), sizeof(info->szOwnerName));
				info->item = this;
				SetOwnershipEvent(event_create(ownership_event, info, PASSES_PER_SEC(iSec)));
				TPacketGCItemOwnership p;
				p.bHeader = HEADER_GC_ITEM_OWNERSHIP;
				p.dwVID = m_dwVID;
				strlcpy(p.szName, RonarkGuild->GetName(), sizeof(p.szName));
				PacketAround(&p, sizeof(p));
			}
			else
			{
...
bool CItem::IsStone() { return (GetType() == ITEM_METIN && GetSubType() == METIN_NORMAL); }
#endif
