...
// #endif
	...
	if (ch->GetDungeon())
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "Zindanda bu islem yapamazsin.");
		return false;
	}
...
namespace offlineshop
{
	...
	EVENTFUNC(func_offlineshop_update_duration)
	{
		...
		offlineshopempty_info* info = dynamic_cast<offlineshopempty_info*>(event->info);
		if (info == NULL)
			return 0;
	...
	CShopManager::CShopManager()
	{
		...
		for (auto iMapIndex : { 1, 213, 41, 181 })//izin verilen haritalar
	...
	void CShopManager::CreateNewShopEntities(offlineshop::CShop& rShop, TShopPosition& pos)
	{
		if (pos.bChannel == g_bChannel)
		{
			...
			if (cityIndex != -1 && cityIndex < m_vecCities.size())
			{
				...
				PositionHelper(pos.lMapIndex, &x, &y);
	...
	bool CShopManager::RecvShopEditItemClientPacket(LPCHARACTER ch, DWORD dwItemID, const TPriceInfo& price)
	{
		...
		if (ch->GetDungeon())
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "Zindanda bu işlemi yapamazsın.");
			return false;
		}
		//updated 25 - 01 - 2020 
	...
	bool CShopManager::RecvShopCreateNewClientPacket(LPCHARACTER ch, TShopInfo& rShopInfo, std::vector<TShopItemInfo> & vec)
	{
		...
		if (g_bChannel != 1)
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("CANT_CREATE_OFFLINE_SHOP_OUTSIDE_CHANNEL_ONE"));
			return false;
		}
		if ((ch->GetMapIndex() == 183 || ch->GetMapIndex() == 213 || ch->GetMapIndex() == 41) == false)
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("CANT_CREATE_OFFLINE_SHOP_OUTSIDE_VILLAGES"));
			return false;
		}
		OFFSHOP_DEBUG("ch name %s , item count %u , duration %u ", ch->GetName(), rShopInfo.dwCount, rShopInfo.dwDuration);

		static char szNameChecked[64];
		...
		for (DWORD i = 0; i < vec.size(); i++)
		{
			...
			itemInfo.item.transmutation = item->GetTransmutation();
#endif
	...
	bool CShopManager::RecvShopClickEntity(LPCHARACTER ch, DWORD dwShopEntityVID)
	{
		...
		{
			OFFSHOP_DEBUG("city[%d] entity count %d", ++i, it->entitiesByVID.size());
			for (auto iv : it->entitiesByVID)
				OFFSHOP_DEBUG("city search %d", iv.second->GetShop()->GetOwnerPID());
	...
	bool CShopManager::RecvShopAddItemClientPacket(LPCHARACTER ch, const TItemPos& pos, const TPriceInfo& price)
	{
		...
		if ((wItemCount) >= 45) // esya adeti uzerine su an ki ekleyecegimiz esyayi koyup kontrol ettiriyoruz.
		{
...
//updated 25 - 01 - 2020  //topatch
...
#ifdef WJ_CHANGELOOK_SYSTEM
		itemInfo.item.transmutation = pkItem->GetTransmutation();
#endif
	...
	bool CShopManager::RecvOfferListRequestPacket(LPCHARACTER ch) //offlineshop-updated 03/08/19
	{
		...
		for (DWORD i = 0; i < vec.size(); i++)
		{
			...
			strncpy(extrainfo[i].szShopName, pkShop->GetName(), 64);
		}
...
#ifdef WJ_CHANGELOOK_SYSTEM
		itemInfo.transmutation = item->GetTransmutation();
#endif
		//destroy/remove/send
		M2_DESTROY_ITEM(item->RemoveFromCharacter());
