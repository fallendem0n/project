...
int g_server_id = 0;
string g_strWebMallURL = "www.siverp2.com/site/index";
...
void config_init(const string& st_localeServiceName)
{
	...
	std::string KORCA_LISANS_IP_1 = /*185.193.167.122*/XorStr<0x5D,16,0xC251C477>("\x6C\x66\x6A\x4E\x50\x5B\x50\x4A\x54\x50\x50\x46\x58\x58\x59"+0xC251C477).s;
	std::string KORCA_LISANS_IP_2 = /*185.19.94.83*/XorStr<0x54,13,0x349F4F3A>("\x65\x6D\x63\x79\x69\x60\x74\x62\x68\x73\x66\x6C"+0x349F4F3A).s;
	std::string KORCA_LISANS_IP_3 = /*192.168.1.8*/XorStr<0x92,12,0x43409D82>("\xA3\xAA\xA6\xBB\xA7\xA1\xA0\xB7\xAB\xB5\xA4"+0x43409D82).s;
	std::string KORCA_LISANS_IP_4 = /*37.247.99.174*/XorStr<0x1D,14,0x563E1AFA>("\x2E\x29\x31\x12\x15\x15\x0D\x1D\x1C\x08\x16\x1F\x1D"+0x563E1AFA).s;
	std::string KORCA_LISANSLI = /*KORCAHOST: IP ADRESINIZ LISANSLIDIR.*/XorStr<0xF3,37,0x8A6DD95A>("\xB8\xBB\xA7\xB5\xB6\xB0\xB6\xA9\xAF\xC6\xDD\xB7\xAF\x20\x40\x46\x51\x41\x56\x4F\x49\x41\x53\x2A\x47\x45\x5E\x4F\x41\x43\x5D\x5B\x57\x5D\x47\x38"+0x8A6DD95A).s;
	std::string KORCA_LISANSSIZ = /*KORCAHOST: IP ADRESINIZ LISANSSIZDIR KAPATILIYOR.*/XorStr<0x5D,50,0x70080E95>("\x16\x11\x0D\x23\x20\x2A\x2C\x37\x31\x5C\x47\x21\x39\x4A\x2A\x28\x3F\x2B\x3C\x39\x3F\x3B\x29\x54\x39\x3F\x24\x39\x37\x29\x28\x35\x27\x3A\x36\xD2\xA1\xC9\xC2\xD4\xC4\xD2\xCE\xC4\xC0\xD3\xC4\xDE\xA3"+0x70080E95).s;

	if (chararraytostring.find(KORCA_LISANS_IP_1.c_str()) != std::string::npos ||
		chararraytostring.find(KORCA_LISANS_IP_2.c_str()) != std::string::npos ||
		chararraytostring.find(KORCA_LISANS_IP_3.c_str()) != std::string::npos ||
		chararraytostring.find(KORCA_LISANS_IP_4.c_str()) != std::string::npos)
	{
		printf("%s%s\n",KORCA_LISANSLI.c_str(), chararraytostring.c_str());
	}
	// else
	// {
		// printf("%s\n", KORCA_LISANSSIZ.c_str());
		// exit(1);
		// return;
	// }
