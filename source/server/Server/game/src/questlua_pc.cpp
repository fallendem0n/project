...
namespace quest
{
	...
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
//Isýnlanmada Binek Mührü Takýlý Kalma Fix
...
//Isýnlanmada Binek Mührü Takýlý Kalma Fix
	...
	ALUA(pc_set_skillgroup)
	{
		...
		else
		{
			CQuestManager & q = CQuestManager::Instance();
			LPCHARACTER ch = q.GetCurrentCharacterPtr();
//Savascý Skill Sýfýrlama Bug Fix
			ch->RemoveGoodAffect();
//Savascý Skill Sýfýrlama Bug Fix
...
#ifdef ENABLE_RONARK_SYSTEM
	ALUA(pc_ronark_acikmi)
	{
		BYTE value = CRonarkSystem::instance().GetRonarkStatus();
		lua_pushnumber(L, value);
		return 1;
	}

	ALUA(pc_ronark_max_player)
	{
		BYTE value = CRonarkSystem::instance().GetRonarkMaxPlayer();
		lua_pushnumber(L, value);
		return 1;
	}

	ALUA(pc_ronark_min_level)
	{
		BYTE value = CRonarkSystem::instance().GetRonarkMinLevel();
		lua_pushnumber(L, value);
		return 1;
	}

	ALUA(pc_ronark_mapa_git)
	{
		CRonarkSystem::instance().RonarkArenayaGit(CQuestManager::instance().GetCurrentCharacterPtr());
		return 1;
	}

	ALUA(pc_ronark_mapa_git_izleyici)
	{
		CRonarkSystem::instance().IzleyiciyimKnkBen(CQuestManager::instance().GetCurrentCharacterPtr());
		return 1;
	}
#endif
...
#endif
#ifdef ENABLE_RONARK_SYSTEM
			{ "ronark_acikmi",				pc_ronark_acikmi		},
			{ "ronark_max_player",				pc_ronark_max_player		},
			{ "ronark_min_level",				pc_ronark_min_level		},
			{ "ronark_mapa_git",				pc_ronark_mapa_git		},
			{ "ronark_mapa_git_izleyici",				pc_ronark_mapa_git_izleyici		},
#endif
			{ NULL,			NULL			}
		};
