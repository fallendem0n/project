							...
							case LUCKBOX_VNUM:
								{
									...
									if (1 == quest::CQuestManager::instance().GetEventFlag("luckbox"))
									{
										ChatPacket(CHAT_TYPE_INFO, LC_TEXT("luckbox_inactive"));
										break;
									}
									...
									if (GetLevel() < LUCKBOX_LEVEL)
									{
										ChatPacket(CHAT_TYPE_INFO, LC_TEXT("luckbox_level"));
										break;
									}
									
									if (item->GetSocket(0) >= 5)
									{
										item->SetCount(item->GetCount()-1);
										ChatPacket(CHAT_TYPE_COMMAND,"luckbox_close");
										break;
									}
									
									int reward = item->GetSocket(1);
									int remain = (5 - item->GetSocket(0));
									SetBoxVID(item->GetVID());
									
									ChatPacket(CHAT_TYPE_COMMAND,"luckbox_open");									
									ChatPacket(CHAT_TYPE_COMMAND,"luckbox_info %d %d %d %d",reward, remain, item->GetSocket(0),item->GetSocket(2));									
									break;
								}
						#endif
							...
							case 71051 : // A©ªAc¡Æ¢®
								{
									...
									if (item2->AddRareAttribute() == true){
										ChatPacket(CHAT_TYPE_INFO, LC_TEXT("¨ù¨¬¡Æ©ªAuA¢¬¡¤I ¨ùO¨ù¨¬AI A©¬¡Æ¢® ¥ìC¨úu¨öA¢¥I¢¥U"));
										...
										ChatPacket(CHAT_TYPE_INFO, LC_TEXT("¢¥o AI¡ío AI ¨ú¨¡AIAUA¢¬¡¤I ¨ùO¨ù¨¬A¡í A©¬¡Æ¢®CO ¨ùo ¨ú©ª¨öA¢¥I¢¥U"));
								}
								break;
#else
							case 71051 : // A©ªAc¡Æ¢®
								{
									// A?¡¤¢¥, ¨öI¡Æ¢®¨¡u, ¨¬¡Ì¨¡¢ç©ø©÷ A©ªAc¡Æ¢® ¡íc¢¯e¡¾?Ao
									LPITEM item2;
							...
							case 71052 : // A©ªAc¡Æ©¡
								{
									...
									if (item2->ChangeRareAttribute() == true){
										...
										ChatPacket(CHAT_TYPE_INFO, LC_TEXT("¨¬?¡Æ©¡ ¨öAA©ø ¨ùO¨ù¨¬AI ¨ú©ª¨öA¢¥I¢¥U"));
								}
								break;
#endif
...
//Oto Pot Lag Fix
									...
									{
										ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<Sistem> Bunu yapabilmek i?n 5 saniye beklemelisin!"));
										return false;
									}
//Oto Pot Lag Fix
									if (CArenaManager::instance().IsArenaMap(GetMapIndex()) == true)
					...
					case USE_TALISMAN:
						{
							...
							if (CArenaManager::instance().IsArenaMap(GetMapIndex()) == true)
							{
								ChatPacket(CHAT_TYPE_INFO, LC_TEXT("´ë·Ã Áß¿¡´Â ?Ì¿ëÇ?¼ö ¾ø´Â ¹°Ç°ÀÔ´Ï´Ù."));
								return false;
							}
#ifdef ENABLE_NEWSTUFF
							else if (g_NoPotionsOnPVP && CPVPManager::instance().IsFighting(GetPlayerID()) && !IsAllowedPotionOnPVP(item->GetVnum()))
...
//?tiflenen Taþlar? Tek Seferde Silinmesi Fix
						item->SetCount(item->GetCount() - 1);
//?tiflenen Taþlar? Tek Seferde Silinmesi Fix
						break;
					}
...
#ifdef ENABLE_ITEM_YOKET_SYSTEM
...
#ifdef BLACKMARKET
bool CHARACTER::BlackMarket(TItemPos Cell)
{
    LPITEM item = NULL; 
	if (!CanHandleItem() || IsDead() || IsStun())
		return false;
	...
	if (item->IsEquipped() || item->isLocked() || item->IsExchanging() || this != item->GetOwner())
		return false;

	
	if (quest::CQuestManager::instance().GetEventFlag("blackmarket") == 0)
	{
		ChatPacket(CHAT_TYPE_INFO, "Sistem g?ev y?eticisi taraf?dan kapat?d?");
		return false;
	}
	
	if (IS_SET(item->GetAntiFlag(), ITEM_ANTIFLAG_SELL))
	{
		ChatPacket(CHAT_TYPE_INFO, "Bu itemi satamazs?.");
		return false;
	}
	...
	{
		ChatPacket(CHAT_TYPE_INFO, "Bu iþlem i?n biraz beklemelisin.");
		return false; 
	}

	
	// int* info = CBlackMarket::instance().GetPriceInfo(this, item->GetVnum(), (short)item->GetCount());	
	
	int goldprice = CBlackMarket::instance().GetGoldPrice(this, item->GetVnum()); 
	int wonprice = CBlackMarket::instance().GetWonPrice(this, item->GetVnum()); 
	
	if (goldprice == -1 || wonprice == -1)
	{
		ChatPacket(CHAT_TYPE_INFO, "%s iteminin piyasa bilgisi mevcut de?l.",item->GetName());
		return false;
	}
	
	if (item->GetCount() != 1)
	{
		if (wonprice == 0 && goldprice > 0)
		{
			unsigned long long llMoney;
			llMoney = goldprice * item->GetCount();
			if (llMoney >= (long long)GOLD_MAX)
			{
				llMoney /= 100000000;
				wonprice = (int)llMoney;
				goldprice = 0;
			}
			else
			{
				wonprice = wonprice*item->GetCount();
				goldprice = goldprice*item->GetCount();
	...
	// unsigned long long llMoney = (long long)GetGold()+ (long long)(info[1]);
	unsigned long long llMoney2 = (long long)GetGold()+ (long long)(goldprice);
	
	if (llMoney2 >= (long long)GOLD_MAX)
	{
		ChatPacket(CHAT_TYPE_INFO, "?erinde bu kadar fazla yang taþ?amazs?.");
		return false;
	}
	
	if (GetCheque() + (wonprice) >= 9999)
	{
		ChatPacket(CHAT_TYPE_INFO, "?erinde bu kadar fazla won taþ?amazs?.");
		return false;
	}
	
	unsigned long long givegold = (long long)(goldprice);
	unsigned long long givewon = (long long)(wonprice);
	
	if (givewon > 0)
	{			
		PointChange(POINT_CHEQUE, (givewon), false);
		ChatPacket(CHAT_TYPE_INFO, "%lld won kazand?",(givewon));
	}
	if (givegold > 0)
	{
		PointChange(POINT_GOLD, (givegold), false);
		ChatPacket(CHAT_TYPE_INFO, "%lld yang kazand?.",(givegold));
	}
		...
		if (item->IsOwnership(this))
		{
			...
			else
			{
				...
				if (item->IsRonarkItem())
				{
					...
					len = snprintf(item1link, sizeof(item1link), "item:%x:%lx", item->GetVnum(), item->GetFlag());

					for (i = 0; i < ITEM_SOCKET_MAX_NUM; i++)
						len += snprintf(item1link + len, sizeof(item1link) - len, ":%ld", item->GetSocket(i));

					for (i = 0; i < ITEM_ATTRIBUTE_MAX_NUM; i++) {
						if (i >= item->GetAttributeCount())
							len += snprintf(item1link + len, sizeof(item1link) - len, ":0:0");
						else
							len += snprintf(item1link + len, sizeof(item1link) - len, ":%x:%hd", item->GetAttributeType(i), item->GetAttributeValue(i));
					}
					snprintf(buf, sizeof(buf), "%s - [Lv.%d] : |cffffc700|H%s|h[%s]|h|r kazandi", GetName(),GetLevel(),item1link,item->GetName());
					this->GetGuild()->Chat(buf);
				}
#endif
				...
				if (item->GetType() == ITEM_QUEST)
				{
					if (GetMapIndex() == CRonarkSystem::instance().RONARK_MAP_VNUM)
...
//Partideki ?elerin Topladýð?Eþyalar? ?tiflenmesi Fix
			if (owner)
			{
				if (item->IsStackable() && !IS_SET(item->GetAntiFlag(), ITEM_ANTIFLAG_STACK))
				{
...
//Partideki ?elerin Topladýð?Eþyalar? ?tiflenmesi Fix
			...
			else if (item->IsStone())
			{
				if (!(owner && (iEmptyCell = owner->GetEmptyStoneInventory(item)) != -1))
				{
					...
					if ((iEmptyCell = GetEmptyStoneInventory(item)) == -1)
					{
						owner->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("¼ÒÁöÇÏ°í ÀÖ??¾ÆÀÌÅÛÀÌ ³Ê¹« ¸¹½À´Ï´Ù."));
						return false;
					}
				}
			}
		...
		if (p->bType == ITEM_MATERIAL && p->bSubType == MATERIAL_LEATHER) //upgrade item
		{
			for (int i = 0; i < SPECIAL_INVENTORY_MAX_NUM; ++i)
			{
				...
				if (item->GetVnum() == dwItemVnum && FN_check_item_socket(item))
				{
					...
					{
						if (bMsg)
							ChatPacket(CHAT_TYPE_INFO, LC_TEXT("lCRELU Ca?: %s"), item->GetName());
						return item;
					}
				}
			}
	...
	if (dwItemVnum == 50300)
	{
		for (int i = 0; i < SPECIAL_INVENTORY_MAX_NUM; ++i)
		{
			...
			if (book->GetVnum() == dwItemVnum && book->GetSocket(0) == item->GetSocket(0))
			{
...
#ifdef __ITEM_COUNT_LIMIT__
				WORD bCount2 = MIN(g_bItemCountLimit - book->GetCount(), bCount);
#else
				...
				{
					if (bMsg)
						ChatPacket(CHAT_TYPE_INFO, LC_TEXT("lCRELU Ca?: %s"), book->GetName());
...
//D?üþ?de Kapanmas?Gereken Skiller Fix
				if (IsAffectFlag(AFF_GEOMGYEONG))
					RemoveAffect(SKILL_GEOMKYUNG);
				if (IsAffectFlag(AFF_GWIGUM))
					RemoveAffect(SKILL_GWIGEOM);
...
//Gelinlik ve Smokin Giyiliyken Kost? Giyinme Fix
...
//Gelinlik ve Smokin Giyiliyken Bine? Binme Fix
	if (item->GetType() == ITEM_COSTUME && item->GetSubType() == COSTUME_BODY)
	{
		LPITEM atakanxd = GetWear(WEAR_BODY);
		if (atakanxd && (atakanxd->GetVnum() >= 11901 && atakanxd->GetVnum() <= 11914))
...
bool CHARACTER::CanUnequipNow(const LPITEM item, const TItemPos& srcCell, const TItemPos& destCell) /*const*/
{
	...
	if (ITEM_BELT == item->GetType())
		VERIFY_MSG(CBeltInventoryHelper::IsExistItemInBeltInventory(this), "?ce kemer slotundaki itemleri almal??.");
