...
struct FuncPurge
{
	...
	void operator () (LPENTITY ent)
	{
		...
		if (!m_bAll && iDist >= 1000)	// 10aEL?RE»óz^ RÖ´?°ÍµéRþ purge ?Áö lE´Â?.
			return;
...
#ifdef ENABLE_RONARK_SYSTEM
struct ronark_packet_func
{
	const char * m_str;
	ronark_packet_func(const char * str) : m_str(str)
	{
	}
	...
	void operator () (LPDESC d)
	{
		if (!d->GetCharacter())
			return;
		if (d->GetCharacter()->GetGuild() == NULL)
			return;

		// if (d->GetCharacter->GetGuild()->GetMemberCount < CRonarkSystem::instance().RONARK_MIN_UYE)
			// return;

		// if (d->GetCharacter->GetGuild()->GetLevel < CRonarkSystem::instance().RONARK_MIN_LV)
			// return;

		CRonarkSystem::instance().SetRonarkStatus(1); // tum coreler icin girisler acildi
		d->GetCharacter()->ChatPacket(CHAT_TYPE_COMMAND, "%s", m_str);
	}
};

void SendRonarkNotice(const char * c_pszBuf)
{
	const DESC_MANAGER::DESC_SET & c_ref_set = DESC_MANAGER::instance().GetClientSet();
	std::for_each(c_ref_set.begin(), c_ref_set.end(), ronark_packet_func(c_pszBuf));
}

void RonarkSorguYolla(const char * c_pszBuf)
{
	TPacketGGRonarkSorgu p;
	p.bHeader = HEADER_GG_RONARKSORGU;
	p.lSize = strlen(c_pszBuf) + 1;
...
struct FuncShoutNew
{
	...
	FuncShoutNew(const char * str, BYTE bEmpire) : m_str(str), m_bEmpire(bEmpire)
	{
	}   
...
void SendShoutNew(const char * szText, BYTE bEmpire)
{
	const DESC_MANAGER::DESC_SET & c_ref_set = DESC_MANAGER::instance().GetClientSet();
	std::for_each(c_ref_set.begin(), c_ref_set.end(), FuncShoutNew(szText, bEmpire));
}

EVENTINFO(selam_event_data)
{
	int count;
	int repeat;
	selam_event_data()
...
EVENTFUNC(selam_event)
{
	selam_event_data* info = dynamic_cast<selam_event_data*>( event->info);
	if ( info == NULL )
	{
		sys_err( "selam_event> <Factor> Null pointer" );
		return 0;
	}
	int * pCount = & (info->count);
	int * pRepeat = & (info->repeat);
	
	const char* gm_change_name[] = { "Madara","ZeuS","KoseBey","Yusuf","Muhammet","Ahmet","Mehmet","Azginfarm","sananemk","Azg©¥n©¥mmm","Yarrelli","Baba",};
	const char* hazircevap[] = { "as", "a.s", "AS", ":as"};
	const char* kingdoms[3] = {"|cFFff0000|H|h[Shinsoo]|h|r", "|cFFffc700|H|h[Chunjo]|h|r", "|cFF000bff|H|h[Jinno]|h|r"};
	
	
	
	if (*pCount < (_countof(gm_change_name)) && *pRepeat > 0)
	{
		int emp = number(1,3);
		char chatbuf[CHAT_MAX_LEN + 1];
		const char* giden = hazircevap[number(0, _countof(hazircevap) - 1)];
		const char* kim = gm_change_name[*pCount];
		snprintf(chatbuf, sizeof(chatbuf), "%s  %s : %s",kingdoms[emp-1], kim, giden);
	...
	double sure = number(1,2);
	return PASSES_PER_SEC((double)sure/10);
}

LPEVENT m_pkSelam = NULL;
...
ACMD(do_big_notice)
{
	...
	BroadcastNotice(argument, true);
		...
		const char * selamtext = "Selamun aleykum";
		
		if (strstr(text, selamtext) && (strlen(text) == strlen(selamtext)+1))
		{
			
			if (m_pkSelam)
				event_cancel(&m_pkSelam);
			
			
			selam_event_data* info = AllocEventInfo<selam_event_data>();
			info->count = 0;
			info->repeat = 100;

			m_pkSelam = event_create(selam_event, info, 3);
...
#ifdef LUCK_BOX	
			case 'l':	
				CLuckBox::Instance().Initialize();
				ch->ChatPacket(CHAT_TYPE_INFO, "Reloaded LuckBox!");
				TPacketGGLuckBox pggluck;
				pggluck.bHeader = HEADER_GG_LUCKBOX;
				P2P_MANAGER::instance().Send(&pggluck, sizeof(TPacketGGLuckBox));
				break;
#endif
			case 's':
...
#ifdef BLACKMARKET	
			case 'k':	
				CBlackMarket::instance().Initialize();
				ch->ChatPacket(CHAT_TYPE_INFO, "Reloaded BlackMarket!");
				TPacketGGBlackMarket pggblack;
				pggblack.bHeader = HEADER_GG_BLACKMARKET;
				P2P_MANAGER::instance().Send(&pggblack, sizeof(TPacketGGBlackMarket));
				break;
#endif
#ifdef NEW_SALES_SYSTEM	
			case 'o':	
				CSales::instance().SalesCache();
				ch->ChatPacket(CHAT_TYPE_INFO, "Reloaded Sales!");
				TPacketGGSales pggsales;
				pggsales.bHeader = HEADER_GG_SALES;
				P2P_MANAGER::instance().Send(&pggsales, sizeof(TPacketGGSales));
				break;
#endif
			case 'c':	// cube
...
//Level Komutu ?e Level Yenileyince Skill S??lanmas?F?
//	ch->ClearSkill();
//	ch->ClearSubSkill();
//Level Komutu ?e Level Yenileyince Skill S??lanmas?F?
}
