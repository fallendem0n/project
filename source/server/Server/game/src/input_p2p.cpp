...
int CInputP2P::SendRonarkNot(LPDESC d, const char * c_pData, size_t uiBytes)
{
	TPacketGGRonarkSorgu * p = (TPacketGGRonarkSorgu *) c_pData;

	if (uiBytes < sizeof(TPacketGGRonarkSorgu) + p->lSize)
		return -1;
	...
	char szBuf[256+1];
	strlcpy(szBuf, c_pData + sizeof(TPacketGGRonarkSorgu), MIN(p->lSize + 1, sizeof(szBuf)));
	SendRonarkNotice(szBuf);
	return (p->lSize);
}
#endif
...
#ifdef BLACKMARKET
void CInputP2P::BlackMarket(const char * c_pData)
{
	TPacketGGBlackMarket * p = (TPacketGGBlackMarket *) c_pData;
...
#ifdef LUCK_BOX
void CInputP2P::LuckBox(const char * c_pData)
{
	TPacketGGLuckBox * p = (TPacketGGLuckBox *) c_pData;
...
#endif
#ifdef ENABLE_RONARK_SYSTEM
		case HEADER_GG_RONARKSORGU:
			if ((iExtraLen = SendRonarkNot(d, c_pData, m_iBufferLeft)) < 0)
				return -1;
			break;
#endif
