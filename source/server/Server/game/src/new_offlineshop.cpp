	...
	*/
	...
	CShopItem::CShopItem(LPITEM pItem, const TPriceInfo& sPrice, BYTE bWindowType, DWORD dwID): m_dwID(0)
	{
		...
		if (pItem)
		{
			...
			// m_itemInfo.evolution = pItem->GetEvolution();

#ifdef WJ_CHANGELOOK_SYSTEM
			m_itemInfo.transmutation = pItem->GetTransmutation();
#endif
		}
	...
	LPITEM CShopItem::CreateItem() const
	{
		...
		// item->SetEvolution(m_itemInfo.evolution);

#ifdef WJ_CHANGELOOK_SYSTEM
		item->SetTransmutation(m_itemInfo.transmutation);
#endif
