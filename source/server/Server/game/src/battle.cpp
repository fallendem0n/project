...
int battle_melee_attack(LPCHARACTER ch, LPCHARACTER victim)
{
	...
	{
//Saldýrý Menzili Düzeltmesi Fix
		int max = 325;
//Saldýrý Menzili Düzeltmesi Fix
...
int CalcAttBonus(LPCHARACTER pkAttacker, LPCHARACTER pkVictim, int iAtk)
{
	...
	if (pkVictim->IsNPC())
	{
		...
		{
			iAtk += (iAtk * (pkAttacker->GetPoint(POINT_BOSS_BONUS))) / 100;
		}
		if(pkVictim->IsStone())
		{
			iAtk += (iAtk * (pkAttacker->GetPoint(POINT_METIN_BONUS))) / 100;
		}
		if (pkAttacker->GetDungeon())
		{
			iAtk += (iAtk * (pkAttacker->GetPoint(POINT_DUNGEON_BONUS))) / 100;
		}
#endif
