...
namespace offlineshop
{
	...
	*/
	...
	CSafeboxCache::TSafeboxCacheInfo* CSafeboxCache::LoadSafebox(DWORD dwPID)
	{
...

		{
			...
			while ((row = mysql_fetch_row(pMsg->Get()->pSQLResult)))
			{
				...
				// str_to_number(item.evolution, row[col++]);
	...
	std::string CreateShopCacheInsertItemQuery(DWORD dwOwner, const CShopCache::TShopCacheItemInfo& rItem)
	{
		...
		// len += snprintf(szQuery + len, sizeof(szQuery) - len, " , %u ", rItem.item.evolution);

#ifdef WJ_CHANGELOOK_SYSTEM
		len += snprintf(szQuery + len, sizeof(szQuery) - len, " , %u ", rItem.item.transmutation);
#endif
		//patch 08-03-2020
	...
	{
		static char szQuery[256];
		static char szEscapeString[64 + 32];
		CDBManager::instance().EscapeString(szEscapeString, name, strlen(name));
	...
	std::string CreateSafeboxCacheInsertItemQuery(DWORD dwOwner, const TItemInfoEx& item)
	{
		...
		// len += snprintf(szQuery + len, sizeof(szQuery) - len, " , %u ", item.evolution);

#ifdef WJ_CHANGELOOK_SYSTEM
		len += snprintf(szQuery + len, sizeof(szQuery) - len, " , %u ", item.transmutation);
#endif
		//patch 08-03-2020
	...
	std::string CreateAuctionCacheAddAuctionQuery(const TAuctionInfo& auction)
	{
		...
		// len += snprintf(szQuery + len, sizeof(szQuery) - len, " , %u ", auction.item.evolution);

#ifdef WJ_CHANGELOOK_SYSTEM
		len += snprintf(szQuery + len, sizeof(szQuery) - len, " , %u ", auction.item.transmutation);
#endif
		//patch 08-03-2020
