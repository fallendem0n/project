	...
	//offlineshop_shop_items
	{
		...
		if (pMsg->Get())
		{
			while ((row = mysql_fetch_row(pMsg->Get()->pSQLResult)))
			{
				...
				str_to_number(item.item.transmutation, row[col++]);
#endif
...
#ifdef WJ_CHANGELOOK_SYSTEM
				str_to_number(info.item.transmutation, row[col++]);
#endif
				//patch 08-03-2020
				BYTE bexp = 0;
...
std::string CreateOfflineshopSelectShopItemsQuery()
{
	char szQuery[1024] = "SELECT `item_id`, `owner_id`, `price_yang`, "
#ifdef ENABLE_CHEQUE_SYSTEM
		"`price_cheque`,"
#endif
		" `vnum`, `count` ";
...
std::string CreateOfflineshopSelectAuctionsQuery()
{
	char szQuery[1024] = "SELECT `owner_id`, `duration`, `name`, `init_yang`, `vnum`, `count` ";
	size_t len = strlen(szQuery);
