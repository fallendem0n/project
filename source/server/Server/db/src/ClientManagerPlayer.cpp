...
#if defined(ENABLE_SPECIAL_STORAGE) && defined(ENABLE_SWITCHBOT)
					"FROM item%s WHERE owner_id=%d AND (window in ('INVENTORY','EQUIPMENT','DRAGON_SOUL_INVENTORY','BELT_INVENTORY','UPGRADE_INVENTORY','BOOK_INVENTORY','STONE_INVENTORY','SWITCHBOT'))",
#else
...
#if defined(ENABLE_SPECIAL_STORAGE) && defined(ENABLE_SWITCHBOT)
		snprintf(queryStr, sizeof(queryStr), "DELETE FROM item%s WHERE owner_id=%d AND (window in ('INVENTORY','EQUIPMENT','DRAGON_SOUL_INVENTORY','BELT_INVENTORY','UPGRADE_INVENTORY','BOOK_INVENTORY','STONE_INVENTORY','SWITCHBOT'))", GetTablePostfix(), pi->player_id);
#else
