import uiCommon, snd, chat, app, net, player, item, wndMgr, mouseModule, localeInfo, constInfo, ui, background
...
class ChestDropWindow(ui.ScriptWindow):
	...
	def SetOpenAmount(self, amount):
		...
		self.openChestButtonSingle.SetText("%d adet sandik ac" % self.openAmount)
		self.openChestButtonMultiple.SetText("%d adet sandik ac" % self.openAmount)
	...
	def OnChangeOpenAmount(self):
		openTemp = int(self.openCountController.GetSliderPos() * 100)
		if openTemp == 0:
			self.SetOpenAmount(1)
		else:
	...
	def OnClickOpenChest(self):
		if self.remaining_time > app.GetTime():
			return
		if self.invItemPos == -1:
			return

		self.remaining_time = app.GetTime() + 0.2
	...
	def RefreshItems(self, chestVnum):
		...
		if not self.chestDrop.has_key(self.currentChest):
			chat.AppendChat(chat.CHAT_TYPE_INFO, "Bu sandik acilamaz!")
			self.Close()
			return
