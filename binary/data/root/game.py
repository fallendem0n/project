...
class GameWindow(ui.ScriptWindow):
	def __init__(self, stream):
		...
		self.chequetogold = uimoneytranslate.ChequeToGoldWindow()
		...
		import uiRonark
		self.ronarkWarScoreBoard = uiRonark.RonarkWarScoreBoard()
...
######################
	...
	def __BuildKeyDict(self):
		...
		onPressKeyDict[app.DIK_F8]		= lambda : self.PriceTransWindowOpen()
		onPressKeyDict[app.DIK_F9]	= lambda : self.interface.OpenDeleteItem()
	...
	def BINARY_NEW_AddAffect(self, type, pointIdx, value, duration):
		...
		elif 209 == type: #ye?il bar
			self.playerGauge.RefreshGuageColor("lime")
			self.interface.HPPoisonEffectShow()
			self.interface.PartyPoisonGuageShow() #ye?il bar end
	...
	def BINARY_NEW_RemoveAffect(self, type, pointIdx):
		...
		if chr.NEW_AFFECT_DRAGON_SOUL_DECK1 == type or chr.NEW_AFFECT_DRAGON_SOUL_DECK2 == type:
			...
			self.interface.PartyPoisonGuageHide() # ye?il bar end
	...
	if app.ENABLE_TARGET_HP_SYSTEM:
		def SetPCTargetBoard(self, vid, name, hpPercentage, actualHP, maxHP):
	...
	def ShowMapName(self, mapName, x, y):
		...
		try:
			...
			self.mapNameTextLine.SetPosition((wndMgr.GetScreenWidth() - 130) / 2, 175)
		except:
	...
	def OnMouseLeftButtonUp(self):
		...
		if mouseModule.mouseController.isAttached():
			...
			elif app.ENABLE_SPECIAL_STORAGE and player.SLOT_TYPE_UPGRADE_INVENTORY == attachedType or player.SLOT_TYPE_BOOK_INVENTORY == attachedType or player.SLOT_TYPE_STONE_INVENTORY == attachedType:
				self.__PutItem(attachedType, attachedItemIndex, attachedItemSlotPos, attachedItemCount, self.PickingCharacterIndex)
	...
	def __DropItem(self, attachedType, attachedItemIndex, attachedItemSlotPos, attachedItemCount):
		...
		if player.SLOT_TYPE_INVENTORY == attachedType and player.IsEquipmentSlot(attachedItemSlotPos):
			...
			if player.SLOT_TYPE_INVENTORY == attachedType:
				...
				dropItemName = item.GetItemName()
				questionText = localeInfo.HOW_MANY_ITEM_DO_YOU_DROP(dropItemName, attachedItemCount)
				itemQuestionDialog = uiCommon.QuestionDialog()
				itemQuestionDialog.SetText(questionText)
				itemQuestionDialog.SetAcceptEvent(lambda arg=True: self.RequestDropItem(arg))
				itemQuestionDialog.SetCancelEvent(lambda arg=False: self.RequestDropItem(arg))
				itemQuestionDialog.Open()
				itemQuestionDialog.dropType = attachedType
				itemQuestionDialog.dropNumber = attachedItemSlotPos
				itemQuestionDialog.dropCount = attachedItemCount
	...
	def RequestDropItem(self, answer):
		...
		if answer:
			...
			elif app.ENABLE_SPECIAL_STORAGE and player.SLOT_TYPE_UPGRADE_INVENTORY == dropType or player.SLOT_TYPE_BOOK_INVENTORY == dropType or player.SLOT_TYPE_STONE_INVENTORY == dropType:
	...
	def OnUpdate(self):
		...
		if constInfo.EFSUNBOT == 1:
			self.interface.ToggleSwitchbotWindow()
			constInfo.EFSUNBOT = 0
		if 1 == constInfo.OPEN_SEARCH_SYSTEM:
			constInfo.OPEN_SEARCH_SYSTEM = 0
			self.OnClickSearch()

		if 1 == constInfo.CHEQUE_TO_GOLD_INFO_OPEN:
			constInfo.CHEQUE_TO_GOLD_INFO_OPEN = 0
			self.PriceTransWindowOpen()
			
		if 1 == constInfo.OPEN_BOSS_MANAGER:
			constInfo.OPEN_BOSS_MANAGER = 0
			self.Boss_Open_Window()

		if 1 == constInfo.OPEN_KYGN_SYSTEM_OPEN:
			constInfo.OPEN_KYGN_SYSTEM_OPEN = 0
			self.__ItemSilTest()

		if 1 == constInfo.OPEN_NPC_SHOP:
			constInfo.OPEN_NPC_SHOP = 0
			self.npcac()
			
		if constInfo.EFSUNBOT_GUI == 1:
			self.__ClickSaplingButton()
			constInfo.EFSUNBOT_GUI = 0
	...
	if app.LUCK_BOX:
		def luckbox_open(self):
			if self.interface:
				self.interface.OpenLuckBox()
				
				
		def luckbox_close(self):
			if self.interface:
				self.interface.CloseLuckBox()
				
		def luckbox_info(self, item, counter, needed, itemcount):
			self.interface.SetLuckBox(item, counter, (int(needed)+1), itemcount)
	...
	def __ServerCommand_Build(self):
		...
		serverCommandList["RonarkInfo"] = self.RonarkInfo
		serverCommandList["ronark_open"] = self.Ronark_Open
		...
		if app.BLACKMARKET:
			serverCommandList.update({
				"blackmarket_insert"	: self.blackmarket_insert,
				"blackmarket_clear"	: self.blackmarket_clear,
				"blackmarket_icon"			: self.interface.BlackMarketShowIcon,
			})
		...
		if app.NEW_SALES_SYSTEM:
			serverCommandList.update({
				...
				"SalesShowIcon"			: self.interface.SalesShowIcon,
			})
		...
		if app.LUCK_BOX:
			serverCommandList.update({
				"luckbox_open"			: self.luckbox_open,
				"luckbox_info"			: self.luckbox_info,
				"luckbox_close"			: self.luckbox_close,
			})
	...
	def Ronark_Open(self):
		if self.ronark_warp_player:
			self.ronark_warp_player.Close()
			
		ronark_warp_player = uiCommon.QuestionDialogRonark()
		ronark_warp_player.SetAcceptEvent(lambda arg=1: self.Ronark_flow(arg))
		ronark_warp_player.SetObserverEvent(lambda arg=2: self.Ronark_flow(arg))
		ronark_warp_player.SetCancelEvent(lambda arg=0: self.Ronark_flow(arg))
		ronark_warp_player.Open()
		self.ronark_warp_player = ronark_warp_player

	def Ronark_flow(self, answer):
		if not self.ronark_warp_player:
			return

		if answer == 1:
			net.SendChatPacket("/ronark arena")
		elif answer == 2:
			net.SendChatPacket("/ronark observer")
			
		self.ronark_warp_player.Close()
		self.ronark_warp_player = None

	def RonarkInfo(self, list, id, name, damage, member, maxcount, observer):
		if self.ronarkWarScoreBoard:
			if not self.ronarkWarScoreBoard.IsShow():
	...
	if app.BLACKMARKET:
		def blackmarket_insert(self, vnum, won, yang):
			constInfo.blackmarket.append("#"+str(vnum)+"#"+str(won)+"#"+str(yang))
			
		def blackmarket_clear(self):
			constInfo.blackmarket = []
	...
	def PriceTransWindowOpen(self):
		self.chequetogold.Show()
	...
	if app.ENABLE_SWITCHBOT:
		...
		def BINARY_SwitchBotSpeedReload(self, speed):
			if self.interface:
				self.interface.SwitchBotSpeedReload(speed)
