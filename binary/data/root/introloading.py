	...
	def Open(self):
		...
		try:
			...
			if constInfo.LOAD_CURTAIN == 1:
				...
				self.GetChild("Top_Line").Show() ## E?er bu blokları kullanmıyorsanız silin.
				self.GetChild("Bottom_Line").Show() ## E?er bu blokları kullanmıyorsanız silin.
			else:
				self.loadingImage.Hide()
				self.GetChild("GageBoard").Hide()
				self.GetChild("Top_Line").Hide() ## E?er bu blokları kullanmıyorsanız silin.
				self.GetChild("Bottom_Line").Hide() ## E?er bu blokları kullanmıyorsanız silin.
