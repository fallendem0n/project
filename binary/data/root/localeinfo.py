...
if app.PRICE_TYPE:
	SHOP_ERROR_DICT["NOT_ENOUGH_MONEY_EX_EP"] = SHOP_NOT_ENOUGH_MONEY_EX_EP
	SHOP_ERROR_DICT["NOT_ENOUGH_ITEM"] = SHOP_NOT_ENOUGH_ITEM

if app.SHOP_STOK:
	SHOP_ERROR_DICT["NOT_ENOUGH_STOK"] = SHOP_NOT_ENOUGH_STOK
...
SHOP_TYPE_MONEY = "  |Eemoji/money|e"
SHOP_TYPE_CHEQUE = "  |Eemoji/won|e"

def NumberToStringAsType(value, type):
	return "{:0,.0f}".format(value).replace(',', '.') + type
...
def NumberToMoneyStringEP(n) :
	...
	return "%s %s" % ('.'.join([ i-3<0 and str(n)[:i] or str(n)[i-3:i] for i in range(len(str(n))%3, len(str(n))+1, 3) if i ]), "EP") 
