...
class TargetBoard(ui.ThinBoard):
	...
	def __init__(self):
		...
		if app.ENABLE_TARGET_HP_SYSTEM:
			...
			hpValues = ui.TextLine()
			hpValues.SetParent(self)
			hpValues.SetPosition(100, 4)
			hpValues.SetText("")
			hpValues.SetOutline()
			hpValues.SetWindowHorizontalAlignRight()
			hpValues.Hide()
	...
	if app.ENABLE_TARGET_HP_SYSTEM:
		def Open(self, vid, name, hpPercentage, actualHP, maxHP):
			if vid:
				...
				if app.ENABLE_TARGET_HP_SYSTEM:
					if player.GetName() != name:
						if not self.hpGauge.IsShow():
							...
							#pcHpGauge.SetPosition(220, 14)
							#pcHpGauge.SetWindowHorizontalAlignRight()
							self.hpGauge.Show()
							self.UpdatePosition()
							self.hpValues.SetPosition(110, 4)
							self.hpValues.SetWindowHorizontalAlignRight()
							self.hpValues.SetHorizontalAlignCenter()
							self.hpValues.Show()

						self.hpGauge.SetPercentage(hpPercentage, 100)
						self.hpValues.SetText(localeInfo.DottedNumber(max(actualHP, 0)) + " / " + localeInfo.DottedNumber(maxHP))

						#percentage = (float(hpPercentage) / float(100) * 100) * (float(173) / float(95))
						#self.hpGauge.SetPercentage(hpPercentage, 100)
					else:
	...
	if app.ENABLE_TARGET_HP_SYSTEM:
		def SetHP(self, hpPercentage, actualHP, maxHP):
			if not self.hpGauge.IsShow():
			...
			#self.hpValues.SetText("%s / %s" % localeInfo.DottedNumber(max(actualHP, 0)), localeInfo.DottedNumber(maxHP))
			self.hpValues.SetText(localeInfo.DottedNumber(max(actualHP, 0)) + " / " + localeInfo.DottedNumber(maxHP))
	else:
