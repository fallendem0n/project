...
import background
import serverInfo
import fingolfin_settings

if fingolfin_settings.MINIMAP_ZAMAN == 1:
...
class MiniMap(ui.ScriptWindow):
	...
	def __init__(self):
		...
		self.tooltipInfo = MapTextToolTip()
		self.tooltipInfo.Show()
		self.tarih = ui.TextLine()
		self.tarih.SetPosition((wndMgr.GetScreenWidth() - 170) / 2, 190)     # Kordinat Yer belirle.
		self.tarih.SetWindowHorizontalAlignCenter()
		self.tarih.SetHorizontalAlignCenter()
		self.tarih.SetFeather()
		self.tarih.SetOutline()
		self.tarih2 = ui.TextLine()
		self.tarih2.SetPosition((wndMgr.GetScreenWidth() - 68) / 2, 190)     # Kordinat Yer belirle.
		self.tarih2.SetWindowHorizontalAlignCenter()
		self.tarih2.SetHorizontalAlignCenter()
		self.tarih2.SetFeather()
		self.tarih2.SetOutline()
		if miniMap.IsAtlas():
			self.tooltipAtlasOpen.SetText(localeInfo.MINIMAP_SHOW_AREAMAP)
		else:
	...
	def OnUpdate(self):
		...
		if fingolfin_settings.MINIMAP_ZAMAN == 1:
			...
			baglad2=st
			self.tarih2.SetText(baglad2)
			self.tarih2.Show()
			aylar={"January":"Ocak","February":"Þubat","March":"Mart","April":"Nisan","May":"Mayys","June":"Haz","July":"Tem","August":"Aðustos","September":"Eylül","October":"Ekim","November":"Kasim","December":"Aralýk"}
			ayx=aylar[aycek]
			gunler={"Monday":"Ptesi","Tuesday":"Sali","Wednesday":"Cars","Thursday":"Per","Friday":"Cuma","Saturday":"Ctesi","Sunday":"Paz"}
			g=gunler[gun]
			baglad=b+gc+b+ayx+b+yilx+b+b
			self.tarih.SetText(baglad)
			self.tarih.Show()
