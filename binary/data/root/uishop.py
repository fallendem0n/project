...
class ShopDialog(ui.ScriptWindow):
	...
	def Open(self, vid):
		...
		if chr.IsNPC(vid):
			shopvnum = chr.GetRaceByVID(int(self.vid))
			if shopvnum == 30000:
				isPrivateShop = True
			else:
				isPrivateShop = False
		...
		if player.IsMainCharacterIndex(vid):
			...
			self.titleName.SetText(localeInfo.SHOP_IS_YOURSELF)
			self.btnBuy.Hide()
			self.btnSell.Hide()
			self.btnClose.Show()
			...
			isMainPlayerPrivateShop = False
			self.titleName.SetText(localeInfo.SHOP_IS_NOT_YOUR_SELF % chr.GetNameByVID(vid))
			self.btnBuy.Show()
			self.btnSell.Show()
			self.btnClose.Hide()
	...
	def SellAttachedItem(self):
		...
		if app.ENABLE_SPECIAL_STORAGE:
			if player.SLOT_TYPE_INVENTORY == attachedSlotType or player.SLOT_TYPE_DRAGON_SOUL_INVENTORY == attachedSlotType or player.SLOT_TYPE_UPGRADE_INVENTORY == attachedSlotType or	player.SLOT_TYPE_BOOK_INVENTORY == attachedSlotType or player.SLOT_TYPE_STONE_INVENTORY == attachedSlotType:
	...
	def UnselectItemSlot(self, selectedSlotPos):
		...
		if not shop.IsPrivateShop():
			if app.SHOP_STOK:
				slotPos = self.__GetRealIndex(selectedSlotPos)
				stok = shop.GetItemStok(slotPos)
	...
	def SelectItemSlot(self, selectedSlotPos):
		...
		if isAttached:
			...
			if app.BUY == curCursorNum or isstock == True:
				self.AskBuyItem(selectedSlotPos)
	...
	def AskBuyItem(self, slotPos):
		...
		if app.PRICE_TYPE:
			if itemPriceType == 0:
				...
				itemBuyQuestionDialog.SetText(localeInfo.DO_YOU_BUY_ITEM(itemName, itemCount, localeInfo.NumberToItemString(itemPrice, itemPriceType)))
		else:
			itemBuyQuestionDialog.SetText(localeInfo.DO_YOU_BUY_ITEM(itemName, itemCount, localeInfo.NumberToGoldString(itemPrice)))
		...
		# itemBuyQuestionDialog.itemCount = itemCount
		# itemBuyQuestionDialog.height = 35
		# itemBuyQuestionDialog.pos = slotPos
		# itemBuyQuestionDialog.itemID = itemIndex
		# itemBuyQuestionDialog.itemPrice = itemPrice
		# if app.ENABLE_CHEQUE_SYSTEM:
			# itemBuyQuestionDialog.itemCheque = itemCheque
			# itemBuyQuestionDialog.Open(itemIndex, slotPos, "Fiyat: " + localeInfo.NumberToMoneyString(itemCheque) + " Won, " + localeInfo.NumberToMoneyString(itemPrice) + " Yang")
		# else:
			# itemBuyQuestionDialog.Open(itemIndex, slotPos, "Fiyat: " + localeInfo.NumberToMoneyString(itemPrice))
