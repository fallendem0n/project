...
class NewListBox(ui.Window):
	...
	def __OnScroll(self, position = None):
		...
		toscr = self.CalcTotalItemHeight() - self.GetHeight() - 8
...
class NewListBoxItem(ui.Window):
	DEFAULT_COLOR = grp.GenerateColor(0.0, 0.0, 0.0, 0.6)
	HOVER_COLOR = grp.GenerateColor(1.3, 1.3, 1.3, 0.1)
	...
	def OnMouseOverIn(self):
		if self.isLeft:
			...
			self.color = NewListBoxItem.HOVER_COLOR
	def OnMouseOverOut(self):
		if self.isLeft:
			self.color = LEFT_COLOR = grp.GenerateColor(0.0, 0.7, 1.1, 0.3)
		else:
			self.color = NewListBoxItem.DEFAULT_COLOR
	def SetColorLeft(self):
		self.color = NewListBoxItem.LEFT_COLOR
		self.isLeft = 1
...
class ShopSearchItem(NewListBoxItem):
	...
	def __init__(self):
		...
		self.LoadDialog()
		self.LoadTexts()
		self.SetColor(NewListBoxItem.DEFAULT_COLOR)
	...
	def LoadDialog(self):
		...
		for i in xrange(3):
			...
			slot.LoadImage("scripts/%s" % (slotImageFileName[i]))
		...
		self.itemImage.SetPosition(16, ShopSearchItem.MARGIN)
		self.itemImage.Show()
		...
		self.itemPriceCheque = ui.TextLine()
		self.itemPriceCheque.SetParent(self)
		self.itemPriceCheque.SetText("")
		self.itemPriceCheque.SetFontName("Arial:%d" % ShopSearchItem.COUNT_FONT_SIZE)
		self.itemPriceCheque.SetHorizontalAlignRight()
		self.itemPriceCheque.Show()
		self.RegisterComponent(self.itemPriceCheque)
		self.__AppendItem()
	...
	def SetSlot(self, itemvnum, itemcount):
		...
		self.BottomLine.SetPosition(0, self.GetHeight())
		self.BottomLine.SetSize(378, 0)
	...
	def __SetData(self, itemName, itemCount, itemWidth, itemHeight):
		...
		priceCheque = self.slotInfo["cheque"]
		priceYangColor = self.GetPriceColor(priceYang)
		if itemCount > 1:
			self.itemCount.SetText("x" + str(itemCount))
			lSpace = 100
		self.itemName.SetText(itemName)
		self.itemPriceYang.SetText(localeInfo.NumberToStringAsType(priceYang, localeInfo.SHOP_TYPE_MONEY))
		self.itemPriceCheque.SetText(localeInfo.NumberToStringAsType(priceCheque, localeInfo.SHOP_TYPE_CHEQUE))
		self.itemCount.SetPosition(50, ShopSearchItem.MARGIN + (itemHeight * 32 - ShopSearchItem.COUNT_FONT_SIZE) / 2 - 10)
		self.itemName.SetPosition(lSpace, ShopSearchItem.MARGIN + (itemHeight * 32 - ShopSearchItem.COUNT_FONT_SIZE) / 2 - 10)
		self.itemOwner.SetPosition(lSpace, ShopSearchItem.MARGIN + (itemHeight * 32 - ShopSearchItem.COUNT_FONT_SIZE) / 2 + 10)
		self.itemPriceYang.SetPosition(350, ShopSearchItem.MARGIN + (itemHeight * 32 - ShopSearchItem.COUNT_FONT_SIZE) / 2-15)
		self.itemPriceCheque.SetPosition(350, ShopSearchItem.MARGIN + (itemHeight * 32 - ShopSearchItem.COUNT_FONT_SIZE) / 2+8)
	...
	def GetPriceColor(self, price):
		if price < 5000000000:
			return 0xFFefecdb
		elif price < 20000000000:
			return 0xFF00ff2a
		elif price < 200000000000:
			return 0xFFf2ff00
		elif price < 1000000000000:
			return 0xFFffa600
		elif price < 5000000000000:
			return 0xFFff0000
		elif price < 99999999999999:
			return 0xFFff00ff
	...
	def __OverInItem(self):
		if not self.tooltipItem.IsShow():
			...
			self.tooltipItem.AddItemData(info["vnum"], sockets, attrs)
...
class Slot(ui.Window):
	...
	def __loadBackground(self, isSold):
		bg = ui.ImageBox()
		bg.LoadImage("offlineshop/slot/grid.tga")
		bg.SetParent(self)
		bg.SetPosition(0,0)
		bg.Show()
		...
		sold = ui.ImageBox()
		sold.LoadImage("offlineshop/slot/satildikanka.tga")
	...
	def SetSlot(self, itemvnum, itemcount):
		iconVnum = itemvnum
		# if app.ENABLE_SKILL_BOOK_ICON_RENEWAL:
			# if iconVnum == 50300:
				# skillIndex = self.slotInfo.get('socket', [0])[0]
				# if skillIndex != 0:
					# iconVnum = 91000+skillIndex
...
class Offer(ui.Window):
	...
	def SetPriceYang(self, yang):
		...
		text.Show()
		text.SetText(localeInfo.NumberToMoneyString(yang, True, True))
		self.priceText = text
...
class ShopListElement(ui.Window):
	...
	def __LoadWindow(self, info):
		...
		if ownerName != "NONAME":
			ownerName = "|cffFFC700%s|r" % (ownerName)
		self.owner_id  = owner_id
...
class NewOfflineShopBoard(ui.ScriptWindow):
	...
	def clear(self):
		...
		self.OpenShopShopOwnerButton = None
	...
	def __loadWindow(self):
		...
		self.DragonBoard		= self.GetChild("MainBoard")
		...
		self.OpenShopShopOwnerButton	= self.GetChild("OpenShopShopOwnerButton")
		...
		self.MyPatternsButton.Hide()
		self.ListOfShopButton.Hide()
		self.SearchHistoryButton.Hide()
		...
		self.CommonInputPriceDlg = uiCommon.MoneyInputDialogOfflineShop()
		...
		self.SearchFilterComboBoxSuggestion.SetPosition(13,50 + 45)
		self.SearchFilterComboBoxSuggestion.SetSize(196,17)
		self.SearchFilterComboBoxSuggestion.Hide()
		...
		self.SearchFilterSavePatternButton.Hide()
		self.SearchFilterResetFilterButton.Hide()

		self.SearchFilterSubTypeComboBox = ui.ComboBox()
		self.SearchFilterSubTypeComboBox.SetParent(self.pageBoards["search_filter"])
		self.SearchFilterSubTypeComboBox.SetPosition(13,50 + 45+75*2)
		self.SearchFilterSubTypeComboBox.SetSize(196,17)
		self.SearchFilterSubTypeComboBox.Hide()
		...
		self.SearchFilterTypeComboBox.SetPosition(13,29 + 45+75*2)
		self.SearchFilterTypeComboBox.SetSize(196,17)
		self.SearchFilterTypeComboBox.Hide()
		...
		for i in xrange(player.ATTRIBUTE_SLOT_NORM_NUM):
			...
			self.SearchFilterAttributeButtons[i].Hide()
	...
	def __MakeCreateShopItemsTable(self):
		...
		table = TableWindowWithScrollbar(384, 306, 12,3, TableWindowWithScrollbar.SCROLLBAR_HORIZONTAL)
		table.SetParent(board)
		table.SetPosition(16, 130)
	...
	def __MakeMyShopItemsTable(self):
		...
		table = TableWindowWithScrollbar(384, 288, 12,3, TableWindowWithScrollbar.SCROLLBAR_VERTICAL)
		table.SetParent(board)
		table.SetPosition(27, 130)
	...
	def __MakeMyShopOffersTable(self):
		board = self.pageBoards["my_shop"]
		table = TableWindowWithScrollbar(417, 80, 1, 5, TableWindowWithScrollbar.SCROLLBAR_VERTICAL)
		table.SetParent(board)
		table.SetPosition(20, 470)
		table.Show()
		self.MyShopOffersTable = table

	def __MakeShopListTable(self):
		board = self.pageBoards["shop_list"]
		table = TableWindowWithScrollbar(417, 448, 1, 23, TableWindowWithScrollbar.SCROLLBAR_VERTICAL)
		table.SetParent(board)
		table.SetPosition(30, 26 + 54)
		table.Show()
	...
	def __MakeOpenAuctionOffersTable(self):
		...
		table = TableWindowWithScrollbar(417, 340, 1, 20, TableWindowWithScrollbar.SCROLLBAR_VERTICAL)
		table.SetParent(board)
		table.SetPosition(16, 209)
		table.Show()
	...
	def __MakeAuctionListTable(self):
		...
		table = TableWindowWithScrollbar(417, 471, 1, 20, TableWindowWithScrollbar.SCROLLBAR_VERTICAL)
		table.SetParent(board)
		table.SetPosition(21, 28 + 50)
		table.Show()
	...
	def __MakeSearchFilterCheckBoxes(self):
		...
		race = {
			"warrior" 	: { "x" : 66, 	"y" : 88 + 45, 	},
			"assassin" 	: { "x" : 66, 	"y" : 114 + 45, 	},
			"sura" 		: { "x" : 139, 	"y" : 88 + 45, 	},
			"shaman" 	: { "x" : 139, 	"y" : 114 + 45, 	},
		}

		for k, v in positions.items():
			checkbox = ui.OffShopCheckBox({'base' : "offlineshop/ibrahimgokalp/quest_checkbox.png", 'tip' : "offlineshop/ibrahimgokalp/quest_checked2.png",})
		...
		for k, v in race.items():
			checkbox = ui.OffShopCheckBox({'base' : "offlineshop/checkbox/%s_base.png"%k , 'tip' : "offlineshop/checkbox/%s_tip.png"%k,})
			checkbox.SetParent(self.pageBoards["search_filter"])
			checkbox.SetPosition(v["x"] , v["y"])
			checkbox.Hide()
			checkbox.Enable()
	...
	def __MakeSearchFilterResultItemsTable(self):
		...
		ItemContainerListBox = NewListBox()
		ItemContainerListBox.SetParent(board)
		ItemContainerListBox.SetSize(381, 325)
		ItemContainerListBox.SetPosition(8, 105)
	...
	def __MakeSearchHistoryTable(self):
		...
		table = TableWindowWithScrollbar(378, 470, 1, 20 , TableWindowWithScrollbar.SCROLLBAR_VERTICAL)
		table.SetParent(board)
		table.SetPosition(13, 32 + 40)
		table.Show()
	...
	def __MakeMyOffersTable(self):
		...
		table = TableWindowWithScrollbar(417, 445, 2, 4, TableWindowWithScrollbar.SCROLLBAR_VERTICAL)
		table.SetParent(board)
		table.SetPosition(18, 50 + 40)
		table.Show()
	...
	def __MakeCreateAuctionSlot(self):
		...
		slot.SetPosition(430 - 18,152 + 20 + 81 + 88)
	...
	def __MakeMyAuctionSlot(self, info):
		...
		slot.SetPosition(339+54,97-73 + 30)
		slot.SetInfo(info)
		slot.Show()
	...
	def __OnClickMyShopPage(self):
		...
		self.DragonBoard.SetSize(423,480)
	...
	def OnClickSearchFilterPage(self):
		self.__OnClickSearchFilterPage()

	def SetOfflineshopPageSearch(self):
		self.pageCategory = "search_filter"
	...
	def __OnClickSearchFilterStartSearch(self):
		...
		if pysettings.IKARUS_NOT_NAME_SEARCH == 1:
			if not self.SearchFilterCheckBoxes["name"].IsEnabled():
				bActiveOne = True
	...
	def OpenShopItem_SetValue( self, key,	index,	*args):
		if key == "id":
			...
			self.ShopItemForSale[index]["boost_t"] = args[0]
			self.ShopItemForSale[index]["boost_v"] = args[1]
			self.ShopItemForSale[index]["boost_tm"] = args[2]
			self.ShopItemForSale[index]["boost_c"] = args[3]
	...
	def OpenShopOwnerItemSold_SetValue( self,  key , index , *args):
		if key == "id":
			...
			self.ShopItemSold[index]["boost_t"] = args[0]
			self.ShopItemSold[index]["boost_v"] = args[1]
			self.ShopItemSold[index]["boost_tm"] = args[2]
			self.ShopItemSold[index]["boost_c"] = args[3]
	...
	def ShopFilterResultItem_SetValue( self,  key, index, *args):
		if key in ( "id", "vnum", "count", "price", "owner", 'trans', 'cheque'):
			...
			self.SearchFilterShopItemResult[index]["boost_t"] = args[0]
			self.SearchFilterShopItemResult[index]["boost_v"] = args[1]
			self.SearchFilterShopItemResult[index]["boost_tm"] = args[2]
			self.SearchFilterShopItemResult[index]["boost_c"] = args[3]
	...
	if ENABLE_CHEQUE_SYSTEM:
		def ShopSafebox_SetValutes(self, yang, cheque):
			...
			self.ShopSafeboxValuteTextCheque.SetText(localeInfo.NumberToCheque(cheque))
	else:
	...
	def ShopSafebox_SetValue(self, key , *args):
		...
		if key in ("id", "vnum", "count", 'trans', 'cheque'):
			elm[key] = args[0]
	...
	def __OnCancelAuctionCreateQuestion(self):
		self.CommonQuestionDlg.Hide()
		self.MyCreateAuctionInfo = []

	def __OnAcceptAuctionCreateQuestion(self):
		info = self.MyCreateAuctionInfo
		offlineshop.SendAuctionCreate(info[0], info[1], info[2], info[3])
		self.CommonQuestionDlg.Hide()
		self.MyCreateAuctionInfo = []

	def __OnAcceptCloseShop(self):
		offlineshop.SendForceCloseShop()
		self.Close()
	...
	def __OnAcceptInputPrice(self):
		...
		if ENABLE_CHEQUE_SYSTEM:
			cheque = self.CommonInputPriceDlg.GetCheque()
			if cheque.isdigit():
				cheque = int(cheque)
			else:
				cheque = 0
	...
	def __SetTooltip(self,info):
		...
		if is_building and not info.get('sold', False):
			...
			self.itemTooltip.AppendTextLine(localeInfo.OFFLINESHOP_BUILDING_LEFT_CLICK_CTRL_REMOVE_ITEM, infocolor, False)
		
		elif self.pageCategory == "open_shop" or self.pageCategory == "search_filter":
			self.itemTooltip.AppendSpace(10)
			self.itemTooltip.AppendTextLine("(|Eemoji/key_lclick|e "+localeInfo.OFFLINESHOP_BUY_ITEM_QUESTION_ACCEPT+")", infocolor)

		elif self.pageCategory == "shop_safebox":
			self.itemTooltip.AppendSpace(10)
			self.itemTooltip.AppendTextLine("(|Eemoji/key_lclick|e" + localeInfo.Envanter_tasi + ")", infocolor)
	...
	def RefreshOpenShopPage(self):
		...
		ownerName	= "NONAME"
		if '@' in name:
			ownerName = name[:name.find('@')]
			ownerName = "|cffFFC700|Hmsg:%s|h%s|h|r" % (ownerName, ownerName)
			name = name[name.find('@')+1:]
		...
		self.OpenShopShopOwnerButton.SetText(ownerName)
		self.OpenShopShopOwnerButton.SAFE_SetEvent(self.OnMouseLeftButtonDown)

	def BindInterface(self, interface):
		self.interface = interface
		self.itemTooltip.BindInterface(interface)
